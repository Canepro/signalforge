...
export async function POST(
  ...
  const status = body.status === "failed" ? "failed" : body.status === "applied" ? "applied" : null;
  ...
  if (!result.ok) {
    ...
    return NextResponse.json({ error: "Apply cannot be recorded from this state", code: "invalid_transition" }, { status: 409 });
  }
  return NextResponse.json({
    action_run: result.row,
    post_fix_job: result.postFixJob,
  });
}
