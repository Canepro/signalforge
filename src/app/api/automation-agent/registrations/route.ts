  ...
  try {
    ...
    try {
      const { row, plainToken, token_prefix } = await storage.withTransaction((tx) =>
        tx.automationAgents.createRegistration(source_id, display_name)
      );
      return NextResponse.json(
        {
      ...
      if (code === "automation_agent_already_registered") {
        return NextResponse.json(
          {
            error: "This source already has an automation-agent registration",
            code: "automation_agent_already_registered",
          },
          { status: 409 }
        );
      }
    ...
    return internalServerErrorResponse(err, "POST /api/automation-agent/registrations");
  }
}
