        ...
        {lanes.map((lane) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-[10px] font-bold uppercase tracking-[0.16em] text-outline-variant">
                  {lane.title}
                </div>
            ...
            <div className="mt-4 space-y-2.5">
              {lane.items.map((item) => (
                <Link
                  ...
                  className="block rounded-lg border border-outline-variant/10 bg-surface-container-lowest px-3 py-3 transition-colors hover:bg-surface-container"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 line-clamp-3 whitespace-pre-wrap break-words text-[11px] leading-relaxed text-on-surface-variant">
                        {item.detail}
                      </div>
                    </div>
                    <div className="shrink-0 text-[10px] font-semibold text-on-surface-variant">
                      {item.created_at_label}
                    </div>
                  </div>
                  <div className="mt-2 break-all text-[10px] text-outline-variant">
                    {item.target_name}
                  </div>
                </Link>
              ))}
