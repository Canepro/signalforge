...
- When environment_context.os indicates macOS and pre_findings include mac.daily_cleanup_* or mac.disk_pressure* rule_ids, prefix each top_actions_now item with an action gate in square brackets: [safe-immediate], [review-required], or [operator-verify]. Use safe-immediate for low-risk reversible steps (for example git worktree prune after confirmation), review-required for policy or owner decisions, and operator-verify for cleanup reruns, privileged host changes, or actions that need workstation-owner approval. For mac-diagnostics runs, the server may replace top_actions_now with deterministic gated recommendations after enrichment; still follow the gate-prefix rule if you return top_actions_now yourself.
...
- Prefix each top_actions_now item with [safe-immediate], [review-required], or [operator-verify] as appropriate.`;
  }
