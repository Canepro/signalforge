...
- Best next implementation move: move into a design-first Phase 9 slice for job-scoped collection parameters so container and Kubernetes jobs stop depending on hidden host-local environment. Recommended design note: [`phase-9-job-scoped-collection-parameters.md`](./phase-9-job-scoped-collection-parameters.md).
