...
- Preferred deployment stance for `signalforge-agent`: long-running hardened service near the execution surface. Today that means a host `systemd` service is the first-class path, and the sibling repo now includes a hardened unit, token-file credential loading, `preflight`, a dry-run installer flow, explicit `kubectl` and kubeconfig pinning, and managed kubeconfig install support for that path. A real `systemd-run --user` smoke now validates the preferred service execution path without requiring root. Container and Kubernetes-native packaging remain follow-on work after the scoped job contract and trust boundaries are explicit.
...
- Remaining hardening follow-on in `signalforge-agent`: polished container-runtime host profile checks, richer install ergonomics for future cluster-side packaging, and continued explicit non-laptop deployment guidance for Kubernetes-capable environments.
