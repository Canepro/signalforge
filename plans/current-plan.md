...
- Remaining hardening follow-on in `signalforge-agent`: richer install ergonomics for future cluster-side packaging, continued explicit non-laptop deployment guidance for Kubernetes-capable environments, and one production-like root-owned host install smoke outside the current dev workstation.
