...
- Preferred deployment stance for `signalforge-agent`: long-running hardened service near the execution surface. Today that means a host `systemd` service is the first-class path, and the sibling repo now includes a hardened unit, token-file credential loading, `preflight`, and a dry-run installer flow for that path. Container and Kubernetes-native packaging remain follow-on work after the scoped job contract and trust boundaries are explicit.
...
- Remaining hardening follow-on in `signalforge-agent`: exponential backoff on network errors, richer install ergonomics for container-capable and Kubernetes-capable runners, and explicit non-laptop deployment guidance for Kubernetes-capable environments.
