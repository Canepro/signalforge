...
4. Continue agent hardening: keep the always-on service stance explicit, preserve the new managed-kubeconfig and user-`systemd` validation path, and carry the stricter container-runtime access checks into a production-like host install smoke and future cluster-side packaging ergonomics.
